...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        std::vector<DataPtr> output_infos;
        output_names.reserve(10);
        for (auto& el: network.getOutputsInfo()) {
            output_names.push_back(el.first);
            output_infos.push_back(el.second);
        }
        network.getOutputsInfo().size();

        output_info->setPrecision(Precision::FP32);
        std::cout << "Configure input & output\n" << output_name << "\n";
        // -----------------------------------------------------------------------------------------------------

        // --------------------------- 4. Loading model to the device ------------------------------------------
        ExecutableNetwork executable_network = ie.LoadNetwork(network, device_name);
        // -----------------------------------------------------------------------------------------------------
        std::cout << "Loading model to the device\n";
        // --------------------------- 5. Create infer request -------------------------------------------------
        InferRequest infer_request = executable_network.CreateInferRequest();
        // -----------------------------------------------------------------------------------------------------
        std::cout << "Create infer request\n";
        // --------------------------- 6. Prepare input --------------------------------------------------------
        /* Read input image to a blob and set it to an infer request without resize and layout conversions. */
        cv::Mat image = imread_t(input_image_path, cv::IMREAD_GRAYSCALE);
        ...
        std::cout << "blob convertion time: " << (std::chrono::duration_cast<std::chrono::microseconds>(stop - start)).count() << std::endl;
        // -----------------------------------------------------------------------------------------------------
        ...
        std::cout << "net process time: " << (std::chrono::duration_cast<std::chrono::microseconds>(stop - start)).count() << std::endl;
//        std::cout << output->size() << std::endl;

        for (auto& name : output_names) {
            Blob::Ptr output = infer_request.GetBlob(name);
            std::cout << "-- " << name << ": " << output->size() << std::endl;

        }


        // writeMatToFile(*output, "result.ext");
